...
void WalkAST::checkLoopConditionForFloat(const ForStmt *FS) {
  ...
  os << "CWE-835. Variable '" << drCond->getDecl()->getName()
  ...
  const char *bugType = "CWE-835. Floating point variable used as loop counter";
...
void WalkAST::checkCall_gets(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_gets,
                     "CWE-242. Potential buffer overflow in call to 'gets'",
                     "Security",
...
void WalkAST::checkCall_getpw(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_getpw,
                     "CWE-447. Potential buffer overflow in call to 'getpw'",
                     "Security",
...
void WalkAST::checkCall_mktemp(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_mktemp,
                     "CWE-377. Potential insecure temporary file in call 'mktemp'",
                     "Security",
...
void WalkAST::checkCall_strcpy(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_strcpy,
                     "CWE-676. Potential insecure memory buffer bounds restriction in "
                     "call 'strcpy'",
                     "Security",
                     "Call to function 'strcpy' is insecure as it does not "
                     "provide bounding of the memory buffer. Replace "
                     "unbounded copy functions with analogous functions that "
                     "support length arguments such as 'strlcpy'.",
                     CELoc, CE->getCallee()->getSourceRange());
}
...
void WalkAST::checkCall_strcat(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_strcpy,
                     ...
                     "provide bounding of the memory buffer. Replace "
                     "unbounded copy functions with analogous functions that "
                     "support length arguments such as 'strlcat'.",
                     CELoc, CE->getCallee()->getSourceRange());
}
...
void WalkAST::checkCall_rand(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  os1 << '\'' << *FD << "' is a poor random number generator. CWE-338";

  SmallString<256> buf2;
  llvm::raw_svector_ostream os2(buf2);
  os2 << "CWE-338. Function '" << *FD
...
void WalkAST::checkCall_random(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_rand,
                     "'random' is not a secure random number generator. CWE-338",
                     "Security",
...
void WalkAST::checkCall_vfork(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_vfork,
                     "CWE-242 .Potential insecure implementation-specific behavior in "
                     "call 'vfork'",
                     "Security",
