...
void DereferenceChecker::reportBug(ProgramStateRef State, const Stmt *S,
  ...
  if (!BT_null)
    BT_null.reset(new BuiltinBug(this, "CWE-476. Dereference of null pointer"));
  ...
  case Stmt::ArraySubscriptExprClass: {
    ...
    os << " results in a null pointer dereference. CWE-476. ";
    break;
  }
  ...
  case Stmt::UnaryOperatorClass: {
    os << "CWE-476. Dereference of null pointer";
  ...
  case Stmt::MemberExprClass: {
    ...
    if (M->isArrow() || bugreporter::isDeclRefExprToReference(M->getBase())) {
      os << "Access to field '" << M->getMemberNameInfo()
         << "' results in a dereference of a null pointer. CWE-476";
...
void DereferenceChecker::checkLocation(SVal l, bool isLoad, const Stmt* S,
  ...
  if (l.isUndef()) {
    if (ExplodedNode *N = C.generateErrorNode()) {
      if (!BT_undef)
        BT_undef.reset(
            new BuiltinBug(this, "CWE-476. Dereference of undefined pointer value"));
