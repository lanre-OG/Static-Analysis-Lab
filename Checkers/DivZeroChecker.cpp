...
void DivZeroChecker::reportBug(const char *Msg,
  ...
  if (ExplodedNode *N = C.generateErrorNode(StateZero)) {
    if (!BT)
      BT.reset(new BuiltinBug(this, "CWE-369.Division by zero"));
...
void DivZeroChecker::checkPreStmt(const BinaryOperator *B,
  ...
  if (!stateNotZero) {
    assert(stateZero);
    reportBug("CWE-369.Division by zero", stateZero, C);
    return;
  }

  bool TaintedD = C.getState()->isTainted(*DV);
  if ((stateNotZero && stateZero && TaintedD)) {
    reportBug("CWE-369. Division by a tainted value, possibly zero", stateZero, C);
    return;
  }
